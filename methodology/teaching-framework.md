...
> 💡 **What is this?** A self-directed Python and tech literacy course for a non-technical professional, built session by session with Claude, to develop enough fluency to communicate confidently with developers and understand the AI landscape. Not about becoming a programmer. About becoming fluent.
...
- **Short sessions, clear stopping points:** Each session covers 2–3 concepts maximum. End before the brain gets full.
...
- **More to add as they emerge** — this list will grow as new techniques surface in sessions.
...
## How It Works: Session Structure & Grounding

### The session flow

1. **Opening quiz** — Retrieval practice on previous session's concepts (off the top of the head, no notes first)
...
4. **Lesson plan doc** — Generated as a .docx at the end of each session, including a Keywords block at the top
...
- **Lesson plans** (per session) — concepts, outcomes, questions, quiz notes. Keywords block at top.
- **Glossary** (running) — vocabulary with definitions and code examples, updated each session
- **Field Guide** (running) — real-world grounding organised by concept, not session. For pre-meeting and pre-interview skimming.
...
- Track quiz results each session and flag shaky concepts for natural reinforcement in future sessions — not immediately, but woven into upcoming lessons organically
- **Session 1 quiz (tested in Session 2):** `print()` output location still fuzzy — doesn't feel concrete yet. Variable concept solid (reasoned independently). Data types solid but same "where does output go" question applies.
- **Session 2 quiz (tested in Session 3):** Functions — defining vs. calling gap filled during retrieval using anchoring (location question). Parameters not yet tested this session.
